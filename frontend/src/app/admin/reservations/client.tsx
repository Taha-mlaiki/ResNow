...
export function ReservationsClient({ reservations }: ReservationsClientProps) {
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-muted/50 border-b">
                                        <tr className="text-left">
                                            <th className="p-4 font-medium text-muted-foreground w-[200px]">User</th>
                                    ...
                                    <tbody>
                                        {reservations.map((res: any) => (
                                            <tr key={res.id} className="border-b last:border-0 hover:bg-muted/5 transition-colors">
                                                <td className="p-4 font-medium">
                                                    {res.participant?.firstName} {res.participant?.lastName}
                                                    <div className="text-xs text-muted-foreground font-normal">{res.participant?.email}</div>
                                                </td>
                                                ...
                                                <td className="p-4">
                                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${res.status === ReservationStatus.CONFIRMED ? 'bg-green-100 text-green-700' :
